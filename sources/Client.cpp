...
int Client::Run() {
  ...
  try {
    ...
    auto const results = resolver.resolve(Params.host, Params.port);
    stream.connect(results);

    http::string_body::value_type body = Params.request_body;

    http::request<http::string_body> req{http::verb::post, Params.target,
                                         Params.version};
